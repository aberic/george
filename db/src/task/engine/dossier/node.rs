        ...
        Self: Sized,
    {
        ...
        if level == 4 {
            ...
            if Vector::is_fill(next_node_seek_bytes.clone()) {
                ...
                next_node_real_seek = node_bytes_seek + next_node_start as u64;
            ...
            if Vector::is_fill(next_node_seek_bytes.clone()) {
                next_node_real_seek = Trans::bytes_2_u64(next_node_seek_bytes)?;
                next_node_bytes = self.node_read(next_node_real_seek, BYTES_LEN_FOR_DOSSIER)?;
            } else {
                // 创建新的结点字节数组
    ...
    fn get_in_node(
        ...
        if level == 4 {
            ...
            if Vector::is_fill(next_node_seek_bytes.clone()) {
                ...
                // 下一结点字节数组
                let next_node_bytes = self.node_read(next_node_real_seek, BYTES_LEN_FOR_DOSSIER)?;
                // 通过当前层真实key减去下一层的度数与间隔数的乘积获取结点所在下一层的真实key
                let next_flexible_key = flexible_key - next_degree * distance;
