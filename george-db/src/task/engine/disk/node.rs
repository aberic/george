...
///
/// 根结点的字节数组长度为16380字节，创建`ge`文件时文件元数据`Metadata`长度为`george_ge::METADATA_SIZE`(52字节)
///
/// 因此`MODIFY_LIMIT_SEEK`长度为`BYTES_LEN_FOR_DISK + george_ge::METADATA_SIZE` = 16380 + 52 = 16432字节
...
impl Node {
    ...
    /// 根据文件路径获取该文件追加写入的空数据
    ///
    /// 空数据由`view版本号(2字节) + view持续长度(4字节) + view偏移量(6字节) + 下一数据地址(8字节)`组成，共20字节
    ///
    /// 直接进行写操作，不提供对外获取方法，因为当库名称发生变更时会导致异常
    ///
    /// #Return
